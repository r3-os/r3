...
fn main() {
    ...
    // Driver-defined test
    println!("cargo:rerun-if-env-changed=CONSTANCE_DRIVER_TEST");
    let selected_test = match env::var("CONSTANCE_DRIVER_TEST") {
        Ok(x) => x,
        Err(env::VarError::NotPresent) => String::new(),
        Err(env::VarError::NotUnicode(_)) => {
            panic!("CONSTANCE_DRIVER_TEST is not a valid UTF-8 string");
        }
    };
